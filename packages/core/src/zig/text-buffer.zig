...
pub const UnifiedTextBuffer = struct {
    ...
    pub fn textToSegments(
    ...
    ) TextBufferError!struct { segments: std.ArrayListUnmanaged(Segment), allocator: std.mem.Allocator, total_width: u32 } {
        ...
        return .{ .segments = segments, .allocator = allocator, .total_width = total_width };
    }
    ...
    fn rebuildLineSpans(self: *Self, line_idx: usize) TextBufferError!void {
        ...
        var active: std.AutoHashMapUnmanaged(usize, void) = .{};
        defer active.deinit(self.global_allocator);
        ...
        for (events.items) |event| {
            ...
            // Process event
            if (event.is_start) {
                try active.put(self.global_allocator, event.hl_idx, {});
            } else {
